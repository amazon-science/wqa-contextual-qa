...
class TestBasePipeline(unittest.TestCase):
    ...
    def	setUp(self):
        self.data = read_dataset('data/wikiqa-test.csv.gz')[:30]
    ...
    def test_trainer(self):
        trainer = AS2Trainer(self.model, debug=True, device='cpu').fit(self.dataloader, self.dataloader)
    ...
    def test_inference(self):
        ...
        pr, cov, thr = precision_curve(labels, scores, questions, title='precsion curve')
...
class TestPositionalPipeline(TestLocalPipeline):
    baseClass = AS2PositionalDataset
    baseModel = PositionalModelForAS2
    baseDataset = AS2PositionalDataset
    tt_size = 5
    ...
    def	test_precision(self):
        ...
        pre, cov, thr =	precision_curve(labels, scores, questions, title='precsion curve')
